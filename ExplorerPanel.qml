...
// puts a transparent click-catcher on every other output so that a click
// anywhere closes it. This explorer is meant to stay open while its owner
// works in an app on another monitor - reading a bind, trying it there,
// coming back - so those twins are gone. A click on this panel's own
// output outside the card still closes it, as does Escape. Keyboard focus
// is per-seat, so while another window is focused the panel is visible but
// inert (no key capture, inhibitor released); clicking the card focuses it
// again.
//
// Everything below this comment is upstream's, kept verbatim so the diff
// recipe in UPSTREAM.md stays small.
//
...
PanelWindow {
  ...
  WlrLayershell.namespace: "fred-keyboard-panel"
  ...
  // Upstream places a transparent click-catcher on every other output here
  // (a Variants over Quickshell.screens) so any click anywhere dismisses the
  // panel. Deliberately absent: see the header comment.
